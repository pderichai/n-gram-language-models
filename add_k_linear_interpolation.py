...
def main(args):
    def get_log_prob(n_gram, model, caches):
        ...
        if n_gram[2:] in caches[0]:
            ...
            unigram_part = cfg.LAMBDA_1_COMBINED * uni_numer / uni_denom
        ...
        if n_gram[1:] in caches[1]:
            ...
            bigram_part = cfg.LAMBDA_2_COMBINED * bi_numer / bi_denom
            caches[1][n_gram[1:]] = bigram_part

        # tri-gram part
        tri_numer = model[2][n_gram] + cfg.K_COMBINED
        tri_denom = model[1][n_gram[:2]] + cfg.K_COMBINED * vocab_size
        if tri_denom == 0:
            trigram_part = cfg.LAMBDA_3_COMBINED * 1 / vocab_size
        else:
            trigram_part = cfg.LAMBDA_3_COMBINED * tri_numer / tri_denom
    ...
    if args.tune:
        print('tuning lambda hyperparemeters for add-K + linear interpolation n-gram model ...')
        print()
        for lambda_1 in cfg.LAMBDA_1s:
            for lambda_2 in cfg.LAMBDA_2s:
                ...
                for K in K_vals:
                    cfg.K_COMBINED = K
                    cfg.LAMBDA_1_COMBINED = lambda_1
                    cfg.LAMBDA_2_COMBINED = lambda_2
                    if lambda_1 + lambda_2 <= 0.9:
                        lambda_3 = 1 - lambda_1 - lambda_2
                        cfg.LAMBDA_3_COMBINED = lambda_3
                        print('K is', cfg.K_COMBINED)
                        print('lambda 1:', cfg.LAMBDA_1_COMBINED,
                              'lambda 2:', cfg.LAMBDA_2_COMBINED,
                              'lambda 3:', cfg.LAMBDA_3_COMBINED)
